...
"""
Trainer for the cross-attention bottleneck variant of CURE-SwinRefiner.

Save this file to:
    nnunetv2/training/nnUNetTrainer/nnUNetTrainerPriorGatedCrossAttentionRefiner.py

To train:
    nnUNetv2_train DATASET_ID 3d_fullres FOLD \
        -tr nnUNetTrainerPriorGatedCrossAttentionRefiner

The only difference vs nnUNetTrainerPriorGatedSwinRefiner is the network:
    Swin:  self_attn(Q=MRI,  K=MRI,   V=MRI)
    Ours: cross_attn(Q=MRI,  K=prior, V=prior)

All loss weights, augmentation, data loading, and residual logit
composition are inherited unchanged from nnUNetTrainerPriorResidualRefiner
...
from nnunetv2.nets.prior_aware_refiner_crossattn import PriorGatedCrossAttnResidualRefiner
...
class nnUNetTrainerPriorGatedCrossAttentionRefiner(nnUNetTrainerPriorResidualRefiner):
    """
    Cross-attention bottleneck trainer.

    Bottleneck change:
        Query  = MRI encoder features at bottleneck resolution
        Key/Value = prior (coarse + uncertainty + SDF) projected to
                    the same spatial resolution and channel count

    This forces the network to explicitly ask:
        "Given what the prior geometry says, which MRI regions matter?"
    rather than letting MRI and prior mix freely in self-attention.
    """
    ...
    def build_network_architecture(
        ...
        if n_blocks_per_stage is None:
            raise KeyError(
                "Expected n_conv_per_stage or n_blocks_per_stage in arch kwargs."
            )

        network = PriorGatedCrossAttnResidualRefiner(
    ...
    def configure_rotation_dummyDA_mirroring_and_inital_patch_size(self):
        ...
        if self._is_femur_dataset():
            ...
            self.print_to_log_file(
                "Femur dataset detected: disabling mirroring to avoid left/right confusion."
            )
    ...
    def _is_femur_dataset(self) -> bool:
        ...
        names = [
            ...
            str(
                getattr(plans_manager, "plans", {}).get("dataset_name", "")
                if plans_manager is not None else ""
            ),
            str(self.dataset_json.get("name", "")),
        ]
        return any("femur" in n.lower() for n in names)
    ...
    # ── auxiliary losses (identical to Swin trainer) ──────────────────

    @staticmethod
    def _signed_distance_target(
        mask: torch.Tensor, like: torch.Tensor
    ) -> torch.Tensor:
        mask_np = (mask.detach().cpu().numpy() > 0.5).astype(np.float32)
        sdf = np.stack(
            [compute_signed_distance(mask_np[i, 0]) for i in range(mask_np.shape[0])],
            axis=0,
        )
        sdf = torch.from_numpy(sdf[:, None]).to(device=like.device, dtype=like.dtype)
        if sdf.shape[2:] != like.shape[2:]:
            sdf = F.interpolate(
                sdf, size=like.shape[2:], mode="trilinear", align_corners=False
            )
    ...
    def _boundary_band_loss(
        ...
        if boundary_logits.shape[2:] != target_fg.shape[2:]:
            boundary_logits = F.interpolate(
                boundary_logits, size=target_fg.shape[2:],
                mode="trilinear", align_corners=False,
            )
        if sdf_logits.shape[2:] != target_fg.shape[2:]:
            sdf_logits = F.interpolate(
                sdf_logits, size=target_fg.shape[2:],
                mode="trilinear", align_corners=False,
            )
        ...
        return (
            self.boundary_aux_loss_weight * boundary_loss
            + self.sdf_aux_loss_weight    * sdf_loss
        )


# convenience alias
PriorGatedCrossAttentionRefinerTrainer = nnUNetTrainerPriorGatedCrossAttentionRefiner
